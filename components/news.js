   ...
   <div class="news-article-body" id="article-body-content">
        ...
        <div style="margin-top:20px;padding:20px;background:var(--sky);border-radius:12px">
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            <!-- WhatsApp -->
            <a href="https://wa.me/?text=${encodeURIComponent(p.title + '\n\nhttps://mathroneacademy.com/news/' + ((p.category === 'news' || !p.category) ? 'education' : p.category) + '/' + (p.slug || p.id))}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:6px;background:#25d366;color:#fff;padding:8px 14px;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600">
...
const CHANNEL_SETS = {
  reb_cbc: {
    label: 'REB & CBC Updates',
    wa_channel: 'https://whatsapp.com/channel/0029Vb95qH5DzgT8Da2LBX34',
    wa_group:   'https://chat.whatsapp.com/JyPdgVq7itSGVIiYBqhrES',
    tg_channel: 'https://t.me/+7sPwyvcv7HQ3NWU0',
    tg_group:   null,
  },
  scholarship: {
    label: 'Scholarship Alerts',
    wa_channel: 'https://whatsapp.com/channel/0029Vb7ryZdJpe8odVQBKT3i',
    wa_group:   null,
    tg_channel: 'https://t.me/wawulence',
    tg_group:  'https://t.me/+8IYJUE1cOItlMjZk',
  },
  abroad: {
    label: 'Cambridge Pathway',
    wa_channel: 'https://whatsapp.com/channel/0029Vb6PEU3Fy72DLobuGb1Z',
    wa_group:   'https://chat.whatsapp.com/IcGUaIzPqJl1RyDFmf9yvr',
    tg_channel: 'https://t.me/+gbqX8IgKuTAxMDE0',
    tg_group:   'https://t.me/+nLmz4X_8wng0N2Y0',
  },
  general: {
    label: 'Mathrone Academy News',
    wa_channel: 'https://whatsapp.com/channel/0029Vb6tCI2EFeXi8jhfjM2E',
    wa_group:   null,
    tg_channel: 'https://t.me/mathroneacademy',
    tg_group:   null,
  },
}

function getChannelSetForArticle() {
  const category = document.getElementById('news-cat')?.value || ''
  const tags = (document.getElementById('news-tags')?.value || '').toLowerCase()
  if (tags.includes('reb') || tags.includes('cbc')) return CHANNEL_SETS.reb_cbc
  if (category === 'scholarship') return CHANNEL_SETS.scholarship
  if (category === 'abroad') return CHANNEL_SETS.abroad
  return CHANNEL_SETS.general
}
...
function insertWhatsAppCTA() {
  ...
  const set = getChannelSetForArticle();
  ...
  const tgIcon = `<svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>`

  const btn = (href, bg, border, textColor, label, icon, filled) => `
        <a href="${href}" target="_blank"
           style="background:${filled ? bg : '#fff'}; color:${filled ? '#fff' : textColor}; ${filled ? '' : `border:1.5px solid ${border};`} text-decoration:none; padding:${filled ? '7px 13px' : '6px 12px'}; border-radius:8px; font-size:12px; font-weight:700; display:inline-flex; align-items:center; gap:6px;">
          ${icon}
          ${label}
        </a>`

  const buttons = [
    set.wa_channel ? btn(set.wa_channel, '#25d366', '#25d366', '#166534', 'WA Channel', waIcon, true)  : '',
    set.wa_group   ? btn(set.wa_group,   '#25d366', '#25d366', '#166534', 'WA Group',   waIcon, false) : '',
    set.tg_channel ? btn(set.tg_channel, '#2AABEE', '#2AABEE', '#0088cc', 'TG Channel', tgIcon, true)  : '',
    set.tg_group   ? btn(set.tg_group,   '#2AABEE', '#2AABEE', '#0088cc', 'TG Group',   tgIcon, false) : '',
  ].filter(Boolean).join('')
  ...
  const ctaHtml = `
    <div style="padding:16px; background:#f8faff; border:1px solid #e2e8f0; border-radius:14px; margin:24px 0; font-family:'DM Sans', sans-serif;" contenteditable="false">
      <div style="font-size:13px; font-weight:800; color:var(--navy); margin-bottom:12px; display:flex; align-items:center; gap:6px;">
        <i data-lucide="bell-ring" style="width:15px; height:15px; color:#25d366"></i> Join our ${set.label} channel
      </div>
