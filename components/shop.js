...
async function renderAdminShop(){
  ...
  try{
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:16px">
      ${products.map(p=>`
      <div class="card" style="padding:0;overflow:hidden;opacity:${p.is_active?1:0.6}">
        <div style="height:140px;background:var(--sky);position:relative;overflow:hidden">
          ${p.image_url?`<img src="${p.image_url}" alt="${p.name||'Product image'}"loading="lazy" decoding="async" style="width:100%;height:100%;object-fit:cover;filter:${p.is_active?'none':'grayscale(80%)'}"/>`:`<div style="display:flex;align-items:center;justify-content:center;height:100%;font-size:40px;filter:${p.is_active?'none':'grayscale(80%)'}}">${SHOP_CATEGORIES.find(c=>c.id===p.category)?.icon||'🛍️'}</div>`}
          ...
          ${!p.is_active?`<div style="position:absolute;top:8px;left:8px;background:#ef4444;color:#fff;font-size:10px;font-weight:700;padding:4px 8px;border-radius:999px">⛔ Inactive</div>`:''}
        ...
        <div style="padding:14px">
          ...
          <div style="display:flex;gap:6px">
            ...
            <button class="btn btn-ghost btn-sm" style="color:${p.is_active?'var(--orange)':'var(--green)'};background:${p.is_active?'#fef3c7':'#dcfce7'}" title="${p.is_active?'Deactivate':'Activate'}" onclick="toggleProductActive('${p.id}',${p.is_active})"><i data-lucide="${p.is_active?'eye-off':'eye'}" style="width:14px;height:14px"></i></button>
...
async function toggleProductActive(id, currentlyActive){
  try{
    const res = await api(`/shop/products/admin/${id}/toggle-active`,{ method:'PATCH' })
    toast(res.message)
    renderAdminShop()
  }catch(e){ toast(e.message,'err') }
}
